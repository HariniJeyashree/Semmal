...
Modern job preparation is fragmented. Candidates and students struggle to understand their specific skill gaps when targeting roles. Traditional automated screening systems (ATS) rely on rigid keyword matching, filtering out qualified candidates who lack specific phrasing. Additionally, manual evaluation of resume alignment is slow and subjective, leaving candidates without constructive feedback.
...
* **API Rate Limiting & Concurrency Exhaustion (429 Errors):** When running batch evaluations for multiple resumes concurrently, sequential API calls easily exceeded the provider's RPM (Requests Per Minute) limits, resulting in `429 Too Many Requests` API exceptions and failed evaluations.
...
* **Transient Database Connections & Concurrency Collisions:** Concurrently committing evaluation scores and generated interview questions for multiple sessions to our cloud Neon PostgreSQL database frequently caused connection timeouts and transaction collisions.
* **Vector Isolation & Context Leakage:** Ingesting multiple resumes into a global ChromaDB instance threatened cross-user context leakage, where search queries for one job session could retrieve matches from previous candidates' sessions.
...
* **Dynamic Collection Isolation:** Implemented session-scoped ChromaDB collection instantiation. Each user session generates an isolated collection mapped to the specific `job_session_id`, ensuring resume data is segregated and context leakage is prevented.
...
The final result is a responsive, highly secure web application containing a complete candidate career preparation workstation. 
